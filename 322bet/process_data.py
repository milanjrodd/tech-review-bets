...
def process_matches(path: str | pathlib.Path | pathlib.PurePath):
    ...
    for i in range(1, 6):
        df[f"radiant_hero_{i}"] = (
            matches["radiant_team"].str.split(",").str[i - 1].astype(int)
        )
        df[f"dire_hero_{i}"] = (
            matches["dire_team"].str.split(",").str[i - 1].astype(int)
        )
    ...
    for i in range(1, 6):
        df[f"radiant_hero_{i}_winrate"] = df[f"radiant_hero_{i}"].p_apply(
            lambda x: heroes.loc[int(x)][f'{matches["avg_rank_tier"].iloc[0]//10}_win']
            / heroes.loc[int(x)][f'{matches["avg_rank_tier"].iloc[0]//10}_pick']
        )
        df[f"dire_hero_{i}_winrate"] = df[f"dire_hero_{i}"].p_apply(
            lambda x: heroes.loc[int(x)][f'{matches["avg_rank_tier"].iloc[0]//10}_win']
            / heroes.loc[int(x)][f'{matches["avg_rank_tier"].iloc[0]//10}_pick']
        )
