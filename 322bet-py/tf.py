...
def main():
    ...
    data_test_output = np.array(
        [[i] for i in testData['radiant_win'].to_numpy()])
    ...
    print(
        f"Train on {len(trainData)} samples, validate on {len(testData)} samples")
    model.fit(tensor_train_input, tensor_train_output, validation_data=(tensor_test_input,
              data_test_output), epochs=300, batch_size=4800, callbacks=[model_checkpoint_callback])
